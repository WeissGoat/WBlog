...
在接触 Protobuf 之前，最常用的序列化格式莫过于 JSON 和 XML。但在高并发、对带宽和延迟极其敏感的场景（例如微服务内部的 RPC 调用、高频的网络状态同步）下，纯文本格式的瓶颈就会显现。Protobuf 的脱颖而出，主要归功于以下三点：
...
3. **极强的向后/向前兼容性（版本控制神器）**
   在 Protobuf 的 `.proto` 文件中，每个字段都有一个唯一的编号（例如 `id = 1`）。**只要你不改变这些核心编号，就可以随意增加新字段或废弃旧字段**。
